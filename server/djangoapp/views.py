...
def registration(request):
    try:
        ...
        if User.objects.filter(username=username).exists():
            return JsonResponse({"userName": username, "status": "User already registered"}, status=400)
        ...
        return JsonResponse({"status": "Error in registration process"}, status=500)
...
def get_dealerships(request, state="All"):
    try:
        ...
        return JsonResponse({"status": "Error fetching dealerships"}, status=500)
...
def get_dealer_reviews(request, dealer_id):
    try:
        ...
        return JsonResponse({"status": "Error fetching reviews"}, status=500)
...
def get_dealer_details(request, dealer_id):
    try:
        ...
        return JsonResponse({"status": "Error fetching dealer details"}, status=500)
...
def add_review(request):
    if not request.user.is_anonymous:
        try:
            ...
            return JsonResponse({"status": "Error in posting review"}, status=500)
...
def get_cars(request):
    try:
        ...
        return JsonResponse({"status": "Error fetching cars"}, status=500)
